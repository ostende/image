# Embedded file name: /usr/lib/enigma2/python/Navigation.py
...
class Navigation:
    ...
    def __init__(self, wakeupData = None):
        ...
        if not path.exists('/etc/enigma2/.deep'):
            print '=' * 100
            ...
            try:
                ...
                self.lastshutdowntime, self.wakeuptime, self.timertime, self.wakeuptyp, self.getstandby, self.recordtime, self.forcerecord = (int(now),
                 -1,
                 -1,
                 0,
                 0,
            ...
            self.syncCount = 0
            hasFakeTime = (now <= 31536000 or now - self.lastshutdowntime <= 120) and self.getstandby < 2
    ...
    def wakeupCheck(self, runCheck = True):
        ...
        if runCheck and (self.__wasTimerWakeup or config.workaround.deeprecord.value) and now >= self.wakeupwindow_minus and now <= self.wakeupwindow_plus:
            ...
            print "[NAVIGATION] wakeup type is '%s' %s" % ({0: 'record-timer',
              1: 'zap-timer',
              2: 'power-timer',
              3: 'plugin-timer'}[self.wakeuptyp], {0: 'and starts normal',
              1: 'and starts in standby',
              2: 'and starts not in standby'}[self.getstandby])
            ...
            if self.wakeuptyp == 2:
                ...
                if not self.forcerecord:
                    print '[NAVIGATION] timer starts at %s' % ctime(self.timertime)
            cec = self.wakeuptyp == 0 and Screens.Standby.TVinStandby.getTVstandby('zapandrecordtimer') or self.wakeuptyp == 1 and Screens.Standby.TVinStandby.getTVstandby('zaptimer') or self.wakeuptyp == 2 and Screens.Standby.TVinStandby.getTVstandby('wakeuppowertimer')
    ...
    def getRecordingsCheckBeforeActivateDeepStandby(self, modifyTimer = True):
        ...
        if rec or next_rec_time > 0 and next_rec_time - now < 360:
            ...
            if not self.RecordTimer.isRecTimerWakeup():
                f = open('/tmp/was_rectimer_wakeup', 'w')
                f.write('1')
                f.close()
            if modifyTimer:
