# Embedded file name: /usr/lib/enigma2/python/RecordTimer.py
...
class RecordTimerEntry(timer.TimerEntry, object):
    ...
    def __repr__(self):
        ...
        if self.disabled:
            disabled = ', Disabled'
        return 'RecordTimerEntry(name=%s, begin=%s, end=%s, serviceref=%s, justplay=%s, isAutoTimer=%s%s%s)' % (self.name,
         ctime(self.begin),
         ctime(self.end),
         self.service_ref,
         self.justplay,
    ...
    def tryPrepare(self):
        if self.justplay:
            return True
        elif not self.calculateFilename():
            self.do_backoff()
    ...
    def sendactivesource(self):
        if SystemInfo['HDMICEC'] and config.hdmicec.enabled.value and config.hdmicec.sourceactive_zaptimers.value:
    ...
    def activate(self):
        ...
        if next_state == self.StatePrepared:
            if self.messageBoxAnswerPending:
                self.start_prepare = time() + 1
                return False
            if self.justTriedFreeingTuner:
                self.start_prepare = time() + 5
                self.justTriedFreeingTuner = False
                return False
            ...
            if next_state == self.StateRunning:
                ...
                if self.afterEvent == AFTEREVENT.STANDBY or self.afterEvent == AFTEREVENT.AUTO and self.wasInStandby and (not wasRecTimerWakeup or wasRecTimerWakeup and isRecordTime):
                    if not box_instandby and not tv_notactive:
                ...
                if isRecordTime or abs(NavigationInstance.instance.RecordTimer.getNextZapTime() - time()) <= 900:
                    ...
                    self.wasInStandby = False
                    return True
                if abs(NavigationInstance.instance.PowerTimer.getNextPowerManagerTime() - time()) <= 900 or NavigationInstance.instance.PowerTimer.isProcessing(exceptTimer=0) or not NavigationInstance.instance.PowerTimer.isAutoDeepstandbyEnabled():
                    if self.afterEvent == AFTEREVENT.DEEPSTANDBY or wasRecTimerWakeup and self.afterEvent == AFTEREVENT.AUTO and self.wasInStandby or self.afterEvent == AFTEREVENT.AUTO and wasRecTimerWakeup:
    ...
    def timeChanged(self):
        ...
        self.start_prepare = self.begin - config.recording.prepare_time.value
        self.backoff = 0
    ...
    def gotRecordEvent(self, record, event):
        if self.__record_service.__deref__() != record.__deref__():
            ...
            if event == iRecordableService.evRecordWriteError:
                ...
                Notifications.AddPopup(text=_('Write error while recording. %s') % (_('An unknown error occurred!'),
                 _('Disk was not found!'),
                 _('Disk is not writable!'),
                 _('Disk full?'))[err], type=MessageBox.TYPE_ERROR, timeout=0, id='DiskFullMessage')
...
def createTimer(xml):
    ...
    entry = RecordTimerEntry(serviceref, begin, end, name, description, eit, disabled, justplay, afterevent, dirname=location, tags=tags, descramble=descramble, record_ecm=record_ecm, isAutoTimer=isAutoTimer, ice_timer_id=ice_timer_id, always_zap=always_zap, rename_repeat=rename_repeat)
    ...
    def saveTimer(self):
        ...
        for timer in self.timer_list + self.processed_timers:
            ...
            list.append(' afterevent="' + str(stringToXML({AFTEREVENT.NONE: 'nothing',
             ...
             AFTEREVENT.AUTO: 'auto'}[timer.afterEvent])) + '"')
